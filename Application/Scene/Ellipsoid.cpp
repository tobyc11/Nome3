...
DEFINE_META_OBJECT(CEllipsoid)
{
    BindPositionalArgument(&CEllipsoid::RadiusX, 1, 0);
    BindPositionalArgument(&CEllipsoid::RadiusY, 1, 1);
    BindPositionalArgument(&CEllipsoid::ThetaMax, 1, 2);
    BindPositionalArgument(&CEllipsoid::PhiMin, 1, 3);
    BindPositionalArgument(&CEllipsoid::PhiMax, 1, 4);
    BindPositionalArgument(&CEllipsoid::ThetaSegs, 1, 5);
    BindPositionalArgument(&CEllipsoid::PhiSegs, 1, 6);
}

void CEllipsoid::UpdateEntity()
{
    if (!IsDirty())
        return;
    ...
    float radiusX = RadiusX.GetValue(1.0f);
    float radiusY = RadiusY.GetValue(1.0f);
    ...
    for (int j = 0; j <= numCrossSections; j++) {
      ...
      for (int i = 0; i <= n; i++) {
          ...
          float x = radiusX * cosf(phi);
          float y = radiusY * sinf(phi);
          float z = 0;

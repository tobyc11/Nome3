...
```
# Qt installation 
Unfortunately, the more recent qt5 packages are not compatible with NOME3 (5.15 is confirmed to not work, not sure about the newer ones). Vcpkg doesn't have a good version control as it essentially forces users to install the most recent version, which is incompatible as discussed. Thus, as a temporary solution, let's download Qt from the Qt website directly. 
